...
public class UpdateKanbanColumnTests(IntegrationTestWebAppFactory factory) : AuthorizedIntegrationTest(factory) {
    private readonly IntegrationTestWebAppFactory _factory = factory;

    [Fact]
    public async Task Should_Update_Kanban_Column_And_Return_NoContent() {
        ...
        var updateRequest = new UpdateKanbanColumnRequest("Completed");
        var responseFromUpdate = await test.AuthorizedHttpClient
            .PutAsJsonAsync($"/api/organisations/{content?.OrganisationId}/projects/{contentProject!.ProjectId}/columns/{contentFromKanbanCreation!.KanbanColumnId}", updateRequest);
