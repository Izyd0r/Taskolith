...
public class RemoveFromTaskTests(IntegrationTestWebAppFactory factory, ITestOutputHelper testOutputHelper) : AuthorizedIntegrationTest(factory) {
    private readonly ITestOutputHelper _testOutputHelper = testOutputHelper;
    private readonly IntegrationTestWebAppFactory _factory = factory;

    [Fact]
    public async Task Should_Remove_Member_From_Task_And_Return_NoContent() {
        ...
        await test.AuthorizedHttpClient.PutAsJsonAsync(
            $"/api/organisations/{content?.OrganisationId}/projects/{contentProject!.ProjectId}/tasks/{createTaskResponse!.TaskId}/members",
            assignRequest);
        var responseFromRemoveFromTask = await test.AuthorizedHttpClient.DeleteAsync($"/api/organisations/{content!.OrganisationId}/projects/{contentProject.ProjectId}/tasks/{createTaskResponse.TaskId}/members/{assignRequest.MemberIds.First()}");
        _testOutputHelper.WriteLine(responseFromRemoveFromTask.ToString());
        responseFromRemoveFromTask.StatusCode.Should().Be(HttpStatusCode.NoContent);
    }
}