...
import { Search, UserPlus, UserX, AlertCircle, Settings } from 'lucide-react'
...
const MemberRow = ({
    ...
    const { mutate: kickThisMember, isPending: isKicking } = useRemoveMember(
        organisationId!,
        member.memberId
    )
...
const MembersPage = () => {
    ...
    useEffect(() => {
        if (memberToEditRoles && members) {
            ...
            if (updatedMember) {
                setMemberToEditRoles(updatedMember)
            }
        }
    ...
    const pageState = useMemo(() => {
        ...
        const currentUserPermissions = currentUserAsMember
            ? currentUserAsMember.roles.reduce(
                (acc: number, role: Role) => acc | role.permissions,
                Permission.Public
            )
            : Permission.Public
    ...
    const handleManageRoles = (member: Member) => setMemberToEditRoles(member)
                                ...
                                {isLoading ? (
                                    ...
                                    pageState.filteredMembers.map((member) => (
                                        <MemberRow
                                            ...
                                            onManageRoles={() => handleManageRoles(member)}
                                        />
                                    ))
                                ) : (
                                    <tr>
